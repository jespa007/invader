...
/*  DETECT.C  Traducci¢n Espa¤ol: Oswaldo Varela C.  */
#include <string.h>
...
/*************************************************************************/
static int get_setting(char *str, char id, int hex, int *value){
	char *paramstart;
	char buf1[128];
	char buf2[128];
	strcpy(buf1, str);
	if (strchr(buf1, id) != NULL){
		...
		if (strchr(paramstart, ' ') != NULL)
			*(strchr(paramstart, ' ')) = '\0';
		if (hex) strcpy(buf2, "0x");
			else strcpy(buf2, "");
		strcat(buf2, paramstart);
		*value = (int)strtoul(buf2, NULL, 0);
		return(TRUE);
	}
	else{
		value = 0;

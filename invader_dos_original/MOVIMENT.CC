 ...
 if(!EsNul())
 {
  ...
  else
  {
   ...
   switch(PActual -> condicio)
   {
    ...
    case AUTO2: if((((*X)+(dimensio>>1))<(MYPOSX+8))&&( ((*X)+(dimensio)) < ((AMPLADA_PANTALLA)) ) ) (*X) += velx;
                else if((((*X)+(dimensio>>1))>(MYPOSX+8))&&((*X)>=0)) (*X) -= velx;
                break;
    case MANUAL: if(letras[AMUNT]&&((*Y)>0)) (*Y) -= 2;
                 if(letras[AVALL]&&(((*Y)+dimensio)<ALTURA_PANTALLA-8)) (*Y) += 2;
                 if(letras[ESQUERRA]&&((*X)>0)) (*X) -= 2;
                 if(letras[DRETA]&&(((*X)+16)<AMPLADA_PANTALLA)) (*X) += 2;

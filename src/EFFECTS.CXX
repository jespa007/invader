...

{
 ...
 for( i = 0; i < NBR_STARS; i++ )
  ...
  PutPixel( Star2D[ i ].x, Star2D[ i ].y, 0 ,segment);
...
void DrawStars(word segment )
{
 ...
 for( i = 0; i < NBR_STARS; i++ )
 {
  ...
  if((Star2D[i].x<320)&(Star2D[i].x>=0)&(Star2D[i].y<200)&(Star2D[i].y>=0))
  PutPixel( Star2D[ i ].x, Star2D[ i ].y, Star2D[ i ].Plane + 16 ,segment);
...
void Setup(void)
{
 ...
 {
  Stars[Loop].X=random(320)-160;// Valors de -160 a +160.
  Stars[Loop].Y=random(200)-100;// Valors de -100 a +100.
  Stars[Loop].Z=Loop;
 }
}
...
void Dibuixar_Estrelles(word VSCREEN)
{
 ...
 for(Loop=1;Loop<=maxstars;Loop++)
 {
  ...
  if((X>1)&(X<320)&(Y>1)&(Y<200))
  {
   ...
   if(Stars[Loop].Z>400) PutPixel(X,Y,5,VSCREEN); else
   if(Stars[Loop].Z>300) PutPixel(X,Y,6,VSCREEN); else
   if(Stars[Loop].Z>200) PutPixel(X,Y,7,VSCREEN); else
   if(Stars[Loop].Z>100) PutPixel(X,Y,8,VSCREEN); else
   PutPixel(X,Y,9,VSCREEN);
  }
 }
}

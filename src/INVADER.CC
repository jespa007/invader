...
tInvader::tInvader()
{
 ...
 MyPosY=ALTURA_PANTALLA; MyPosX=112;
 MyPowerUp = 1;
 MySpeedUp = 1;
    ...
    while(!PActual -> Enemic.ActualEsNul())
    {
     ...
     switch(PActual->Enemic.RegaletActual())
     {
      case ITEM1:AfegirItem(PActual->x+x,PActual->y+y,ITEM1,46);
                 ...
                 if(SB) {start_sound(sonidos[23],23,OFF);}break;
  ...
  while(!PActual->Enemic.ActualEsNul())
  {
   ...
   if(HiHaInterseccio(PActual->x+EX,PActual->y+EY,x,y,Sprites[PActual->Enemic.SpriteActual()].dimx,dimensio)&&(Sprites[PActual->Enemic.SpriteActual()].impactable))
   {
    ...
    else
    {
     ...
     if(!PActual->Enemic.ConjuntEsNul())PActual->Enemic.AfegirObjecte(Objectes[IMPACTE],EX - offsetdispar,EY-offsetdispar,0,INFINIT);
    }
    ...
    if(!PActual->Enemic.DecrementarVitalitatActual(power))
    {
     switch(PActual->Enemic.RegaletActual())
     {
      case ITEM1:AfegirItem(PActual->x+EX,PActual->y+EY,ITEM1,46);
                 ...
                 CrearExplosio(PActual->x+EX,PActual->y+EY,METEORIT_DESTRUIT);         if(SB) {start_sound(sonidos[23],23,OFF);}break;
      case ITEM2:    
                 CrearExplosio(PActual->x+EX,PActual->y+EY,METEORIT_DESTRUIT);         if(SB) {start_sound(sonidos[23],23,OFF);}break;
      ...
      case BALA3:CrearDispar(PActual->x+EX+8,PActual->y+EY+16,7,38,CUA_DISPARS_ENEMICS);
                 ...
                 CrearExplosio(PActual->x+EX,PActual->y+EY,EXPLOSIO1);if(SB) {start_sound(sonidos[23],23,OFF);}break;
      default:   
                 CrearExplosio(PActual->x+EX,PActual->y+EY,EXPLOSIO1);
                 if(SB) {start_sound(sonidos[23],23,OFF);}
                 break;
...
void tInvader::Actualitzar() // Checkejat i dibuixat de totes les naus...
{
 ...
 if((PActual = EnemicsTerrestres)!= NULL)
 {
  ...
  if(!(PActual->Enemic.ConjuntEsNul())&&!(PActual->Ruta.EsNul())&&((PActual->x+Sprites[PActual->Enemic.DimensioMax()].dimx)>=0)&&(PActual->x<=AMPLADA_PANTALLA+4)&&((PActual->y+Sprites[PActual->Enemic.DimensioMax()].dimx)>=0)&&(PActual->y<=ALTURA_PANTALLA))
  {
  ...
  if(!(SequenciaDestruccioBoss&&PActual->tipus==BOSS1))
  {
   ...
   if(!(SequenciaDestruccioBoss&&PActual->tipus==BOSS1&&(RetardDestruccioBoss>200)))
   {
    ...
    PActual->Enemic.EscriureObjectes(VSCREEN1);
   }
    PActual=PActual->PSeguent;
 ...
 if((PActual = EnemicsAeris)!= NULL)
 {
  ...
  while(PActual != NULL)
  {
    ...
    if(!(PActual->Enemic.ConjuntEsNul())&&!(PActual->Ruta.EsNul())&&((PActual->x+Sprites[PActual->Enemic.DimensioMax()].dimx)>=0)&&(PActual->x<=AMPLADA_PANTALLA+4)&&((PActual->y+Sprites[PActual->Enemic.DimensioMax()].dimx)>=0)&&(PActual->y<=ALTURA_PANTALLA)&& !( (p=HiHaInterseccio(MyPosX,MyPosY,PActual->x,PActual->y,16,Sprites[PActual->Enemic.SpriteActual()].dimx)&&!(Jo.EsInvencible())&&EsActiu)) )
    {
 ...
 PShotActual = EnemyShots;
  ...
  while(PShotActual != NULL)
  {
   if((!PShotActual->Moviment.EsNul())&&((PShotActual->x+Sprites[PShotActual->Shot.SpriteActual()].dimx)>=0)&&(PShotActual->x<=(AMPLADA_PANTALLA+4))&&((PShotActual->y+Sprites[PShotActual->Shot.SpriteActual()].dimy)>=0)&&(PShotActual->y<=(ALTURA_PANTALLA+4))&&!(  (p=HiHaInterseccio(MyPosX,MyPosY,PShotActual->x,PShotActual->y,16,Sprites[PShotActual->Shot.SpriteActual()].dimx)&&!(Jo.EsInvencible())&&EsActiu)))
   {
    ...
    PShotActual -> Shot.EscriureActual(VSCREEN1);
...
if(SequenciaDestruccioBoss)
{
 ...
 PShotActual = MyShots;
  ...
  while(PShotActual != NULL)
  {
   if((!PShotActual->Moviment.EsNul())&&((PShotActual->x+Sprites[PShotActual->Shot.SpriteActual()].dimx)>=0)&&(PShotActual->x<=(AMPLADA_PANTALLA+4))&&((PShotActual->y+Sprites[PShotActual->Shot.SpriteActual()].dimy)>=0)&&(PShotActual->y<=(ALTURA_PANTALLA+4))&&!(I=Checkit(PShotActual->x,PShotActual->y,PShotActual->Shot.Power(),Sprites[PShotActual->Shot.SpriteActual()].dimx)))
   {
 ...
 PIActual = Items;
  ...
  while(PIActual != NULL)
  {
   if((!PIActual->Moviment.EsNul())&&((PIActual->x+Sprites[PIActual->Item.SpriteActual()].dimx)>=0)&&(PIActual->x<=(AMPLADA_PANTALLA+4))&&((PIActual->y+Sprites[PIActual->Item.SpriteActual()].dimy)>=0)&&(PIActual->y<=(ALTURA_PANTALLA+4))&&!(  (p=HiHaInterseccio(MyPosX,MyPosY,PIActual->x,PIActual->y,16,Sprites[PIActual->Item.SpriteActual()].dimx))&&EsActiu) )
   {
    ...
    PIActual -> Item.EscriureActual(VSCREEN1);
 ...
 while(PExActual != NULL)
 {
  ...
  if(!PExActual -> Explosio.ActualEsNul())
  {
   ...
   PExActual -> Explosio.EscriureActual(VSCREEN1);
 ...
 if(HeTiratBomba)
 {
  if((XBomba>(112-25))&&(YBomba>(96-25))&&(XBomba<(112+25))&&(YBomba<(96+25)))
  {
   ...
   else
   {
    ...
    {
     Bomba.SeguentFrame(0,0);Bomba.EscriureActual(VSCREEN1);
    }
...
if(EsActiu)
{
 ...
 if(letras[DISPAR]&&(Retard_Dispar <= 0))
 {
 ...
 if(letras[BOMBA]&&!HeTiratBomba&&Retard_Bomba<=0&&Bombes>0)
 {
   TirarBomba();
   Bombes--;

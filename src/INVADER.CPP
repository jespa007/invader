...
void main(int argc,char *argv[])
{
 ...
 if(argc>1) if(!strcmp(argv[1],"Gloria")) Cheat = TRUE;
...
// SetPalette(paleta3,0,255);
 ...
 Quit(Motiu0);
                   ...
                   /*-________________________-*\__  ú   ú            ú
  ú       ú     )-/                              \_)   ú      ú
    ú            | (  IMPLEMENTACIO DE FUNCIONS  |_) ú      ú     ú 
 .         ú    )-\                              /_)   ú    ú
   ú   ú           \*_-------------------------_*/
...
if(!escape)
{
 ...
 while(!(escape)&& temps < 2000)
 {
   ...
   if((temps >= 1300) && (temps<1800))
   {
    ...
    OutText(0,165,(byte *)&"  AIXO NOMES ES EL COMENCAMENT !!!            \0",0,0,0,FONTMT,VSCREEN3);
    if(color<63) {ModificarColorPaleta(color,0,0,15);color++;}
   }
